...
def get_best_pixel(pixels):
    ...
    """
    ave_rgb = get_average(pixels)   # the average of R/G/B
    pixel_dist = {}
    for pixel in pixels:
        pixel_dist[pixel] = get_pixel_dist(pixel, ave_rgb[0], ave_rgb[1], ave_rgb[2])
    best_pixel = min(pixel_dist.items(), key=lambda ele: ele[1])
    return best_pixel[0]
